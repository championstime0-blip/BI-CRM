...
# SIDEBAR PARA ALTERNAR ENTRE ATUAL E HISTÓRICO
    ...
    if arquivo:
        ...
        if st.sidebar.button("🚀 SALVAR NO HISTÓRICO"):
            ...
            if client:
                sh = client.open("BI_Historico")
                ws = sh.worksheet("db_snapshots")
                df['snapshot_id'] = datetime.now().strftime("%Y%m%d_%H%M%S")
                df['data_salvamento'] = datetime.now().strftime('%d/%m/%Y %H:%M')
                df['semana_ref'] = semana_sel
                df['marca_ref'] = marca_sel
