...
.card {
    ...
    padding: 24px; border-radius: 16px; border: 1px solid #1e293b; text-align: center; height: 100%;
}
...
.funnel-percent { font-family: 'Orbitron', sans-serif; font-size: 32px; font-weight: 700; color: #22d3ee; }
...
def processar(df):
    ...
    # Garante que não criamos duplicatas após o rename
    df = df.loc[:, ~df.columns.duplicated()].copy()
...
def dashboard(df):
    ...
    with col_funil:
        ...
        st.markdown(f'<div class="funnel-card"><div class="funnel-percent">{taxa:.1f}%</div><div style="color:#94a3b8">TAXA DE AVANÇO REAL</div></div>', unsafe_allow_html=True)

    st.divider()
    subheader_futurista("🚫", "DETALHE DAS PERDAS")
    cl1, cl2 = st.columns(2)
    with cl1: card("Total Perdidos", len(perdidos))
    with cl2: card("Perda: Sem Resposta", len(perda_especifica))
...
semana = st.sidebar.selectbox("Semana de Referência", SEMANAS)
...
if arquivo:
    try:
        ...
        st.markdown(f'<div class="profile-header"><div><span class="profile-label">Responsável:</span> <span class="profile-value">{resp}</span></div><div class="profile-divider"></div><div><span class="profile-label">Equipe:</span> <span class="profile-value">{equipe}</span></div></div>', unsafe_allow_html=True)

        resumo = dashboard(df)

        if st.sidebar.button(f"💾 SALVAR DADOS: {semana}"):
            with st.spinner("Salvando no Google Sheets..."):
                try:
                    ...
                    agora = datetime.now()
                    ws.append_row([agora.strftime('%d/%m/%Y'), agora.strftime('%H:%M:%S'), semana, resp, equipe, resumo['total'], resumo['and'], resumo['perd'], resumo['sr'], f"{resumo['tx']:.1f}%", resumo['top']])
